...
export function AsphaltDetection({ onAreaSelect, showEmployeeTracking = false }: AsphaltDetectionProps) {
  ...
  const drawAreas = useCallback((areas: AsphaltArea[]) => {
    ...
    areas.forEach((area, index) => {
      ...
      ctx.fillStyle = area.manuallyEdited ? `rgba(255, 165, 0, ${alpha})` : `rgba(0, 123, 255, ${alpha})`;
      ctx.fill();

      // Stroke outline
      ctx.strokeStyle = area.manuallyEdited ? '#ff9500' : '#007bff';
      ctx.lineWidth = 2;
      ctx.stroke();
  ...
  }, [selectedArea, editMode, showEmployeeTracking]);
